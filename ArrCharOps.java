...
public class ArrCharOps {
    ...
    public static char[] concat(char[] arr1, char[] arr2) {
        ...
        while (currIndexOfNewarr < newArrLen) {
            for (int i1 = 0; i1 < arr1.length; i1++) {
                newArr[currIndexOfNewarr] = arr1[i1];
                currIndexOfNewarr++;
            }
            for (int i2 = 0; i2 < arr2.length; i2++) {
                newArr[currIndexOfNewarr] = arr2[i2];
                currIndexOfNewarr++;
            }
        }
        return newArr;
    ...
    public static char[] subArray(char[] arr, int beginIndex, int endIndex) {
        // Replace the following statement with your code
        int newArrLen = endIndex - beginIndex;
        char[] newArr = new char[newArrLen];
        int currIndexOfNewarr = 0;
        while (currIndexOfNewarr < newArrLen) {
            for (int i = beginIndex; i < endIndex; i++) {
                newArr[currIndexOfNewarr] = arr[i];
                currIndexOfNewarr++;
            }
        }
        return newArr;
